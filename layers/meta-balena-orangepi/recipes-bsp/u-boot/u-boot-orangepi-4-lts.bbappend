...
DEPENDS += "orangepi-binary-loader orangepi-binary-native"

SRC_URI_append = ""

BALENA_BOOT_PART_orangepi-4-lts = "4"
BALENA_DEFAULT_ROOT_PART_orangepi-4-lts = "5"

do_compile_append() {
    # create bootloader image
    loaderimage --pack --uboot ./u-boot-dtb.bin ${DEPLOY_DIR_IMAGE}/u-boot.img 0x200000 --size 1024 1

    ./tools/mkimage -n ${SOC_FAMILY} -T rksd -d ${DEPLOY_DIR_IMAGE}/orangepi-binary/ddr.bin ${DEPLOY_DIR_IMAGE}/idbloader.bin
    cat ${DEPLOY_DIR_IMAGE}/orangepi-binary/miniloader.bin >>${DEPLOY_DIR_IMAGE}/idbloader.bin
